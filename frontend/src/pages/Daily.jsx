...
export default function Daily() {
  ...
  // Timer ticking
  useEffect(() => {
    if (!isFocusRunning || !focusStartTime) return;

    const interval = setInterval(() => {
      const diff = Math.floor((Date.now() - focusStartTime) / 1000);
      setElapsedSeconds(diff);
    }, 1000);

    return () => clearInterval(interval);
  }, [isFocusRunning, focusStartTime]);
  ...
  useEffect(() => {
    ...
    // Reset focus UI when switching day/week
    setIsFocusRunning(false);
    setFocusStartTime(null);
    setElapsedSeconds(0);
  }, [W, D]);
            ...
            <div className="mt-6 rounded-xl border theme-border theme-card p-4">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                  ...
                  {isFocusRunning && (
                    <span className="text-sm text-white/70 px-3 py-2">
